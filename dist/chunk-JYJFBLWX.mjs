...
function getBasicAppContext() {
  return {
    reactNativeVersion: Platform.constants?.reactNativeVersion ? `${Platform.constants.reactNativeVersion.major}.${Platform.constants.reactNativeVersion.minor}.${Platform.constants.reactNativeVersion.patch}` : void 0
  };
}
...
var StorageManager = class {
  ...
  async getOrCreateDeviceId() {
    ...
    if (!deviceId) {
      deviceId = `${Platform2.OS}-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`;
...
// src/client.ts
import { Platform as Platform3, AppState } from "react-native";
...
var TracekitClient = class {
  ...
  async getAppContext() {
    ...
    try {
      const Application = __require("expo-application");
